...
真实全量重转也已完成：28 条录制、73,215 个采样点、112 段视频（四路共 292,860 帧），默认 50 FPS，task 为 `Plug in the Ethernet cable`。从原始 MCAP 全新生成缓存，再由 LeTools 写出 v2.1；未复用旧的有损中间图像。

| 阶段 | 本次实测耗时 |
|---|---:|
| 数值读取与信号对齐 | 368.19 秒 |
| 表格与逐帧审计写入 | 0.20 秒 |
| 视频解码、CRF 18 编码及完整像素统计 | 1,671.04 秒 |
| LeTools 写出 v2.1（复用 H.264 编码包） | 1.12 秒 |
| 完整转换总耗时（含初始化等开销） | **2,040.70 秒，34 分 0.70 秒** |
| 额外完整验证 | 186.66 秒 |
| 转换与验证合计 | 2,227.37 秒，37 分 7.37 秒 |

完整转换吞吐为 **35.88 个采样点/秒**（四路共 143.51 张图像/秒）；视频处理阶段为 43.81 个采样点/秒。处理平台为 macOS 26.1 / arm64、10 个逻辑 CPU、Python 3.12.12，LeTools data workers=2、video workers=4；准备阶段每个编码器 1 线程、四宫格解码器 2 线程。输入 MCAP 总计 5.912 GB；最终核心数据文件 1.097 GB，另保留同量级可复用缓存。全程含验证的进程峰值 RSS 为 1,132,036,096 字节（约 1.05 GiB）。这些是一次本机实测；上次低画质转换为 2,132.97 秒，本次少用 92.27 秒，不据此保证其他机器或负载下的性能提升。

- LeTools deep validation 零错误、零警告；全部 73,215 行与准备源、旧数据集逐值一致，统计和 task 一致。时间网格间隔严格为 20 ms，处于所有信号交集内；所选视频帧没有来自未来，年龄不超过 100 ms。
- 112 段视频均完整解码，尺寸为 960×540，帧数与表格行数一致，每帧 PTS 等于帧号 / 50；实际 x264 码流确认 CRF 18、无 B 帧。准备缓存与最终视频的编码包完全一致，没有二次有损编码。
- 每条每路首/中/末帧与对应原始四宫格区域比较，共 **336 张**：PSNR 最低 38.98 dB、中位数 40.49 dB、最高 41.91 dB，最大平均绝对像素误差 2.080/255。
- 另对旧诊断使用的完全相同 16 张画面比较，旧版 PSNR 中位数 34.08 dB，新版为 **41.23 dB**。相机、原始帧索引和时间映射均已核对，画质改善不是由选择不同帧造成。
- 一条录制仍对应一个 episode，中间丢帧为 0，首尾静止裁剪合计 204 帧。4 条已移出的异常录制没有进入输出。原始文件大小/修改时间指纹前后一致。
- 结果在开发机 `/Users/xieweikai/Downloads/tianji_dataset/lerobot_v21`；完整验证明细和同帧质量报告分别为 `../tmp/plug-wire-v21-hq-v040-validation.json`、`../tmp/completed-video-quality-v040.json`，清单与计时同时保存到数据集 `meta/tianji_conversion.json`。
