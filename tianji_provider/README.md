...
0.2.0 起，一条录制对应一个 episode，不再拆段。遥操作命令停发时，按左右臂和左右夹爪分别用同一输出帧的 state 填 action；默认裁掉首尾静止部分，保留中途暂停。旧版切段缓存不能复用，必须重新准备。

0.4.0 修复直接转换的图像细节损失：中间帧改为无损 RGB（PPM），不再经过 JPEG。正式训练数据推荐使用下面的准备缓存流程：原始视频解码后直接编码为 H.264，固定 CRF 18、veryfast、yuv420p，LeTools 复用编码结果。

## 使用

生成数据集时 `--task` 必填且不能为空；`--fps` 为目标数据集帧率，默认 **50**，接受 1–1000 的整数。
...
准备过程在视频编码/统计阶段对每个原始四宫格只完整解码一次，同时产生四路 H.264 MP4；对齐阶段另读取时间戳和首帧尺寸。编码固定 CRF 18、veryfast、yuv420p、无 B 帧，每个编码器 1 线程；LeTools 随后直接复用编码视频，不进行第二次有损转码。准备源是已有视频，后续转换命令无需传入图像编码选项。缓存可重复使用，修改任务文本无需重新编码。改变 FPS 或对齐策略需要一个新缓存目录。已有目录不会自动覆盖；中断时没有完成标记的目录不会被误认为有效缓存。

缓存清单保存实际编码设置及对齐、表格写入、视频解码/编码/统计阶段的耗时，也记录每条录制的视频处理耗时。编码器若忽略设置会立即报错，避免静默退回默认画质。总转换速度应包含首次准备缓存和 LeTools 输出两阶段；复用缓存后的封装速度不能代表原始 MCAP 转换速度。

直接转换模式完全只读，按需提供有界、逐像素无损的 RGB PPM `FrameSequence`，可安全重复帧及随机读取；四路相机分别顺序解码原视频，适合少量数据。它计算视频统计时另有一次顺序解码。PPM 不压缩，960×540 单帧约 1.48 MiB；默认 48 帧批次的编码缓冲上限约 71 MiB/相机（重复帧复用同一缓冲），不保留整条轨迹图像。最终有损编码参数由 LeTools 后端决定；当前后端选择 libx264 时默认为 CRF 23，不能把此路径等同于准备缓存的 CRF 18。原来已经生成的 JPEG 中间层视频需要从原始录制重新转换才能恢复细节；此前合法的直接 H.264 准备缓存仍可复用。
...
```bash
tianji-provider check /path/to/recordings --report /path/to/tmp/diagnostics.json
```

检查默认使用 50 FPS 和相同的对齐、命令补齐、首尾裁剪规则；可用 `--fps` 等参数与正式转换保持一致。检查不生成数据集，因此 `--task` 可省略；`prepare`、`inspect` 和转换仍要求填写 task。默认完整解码所有视频，`--no-deep-video` 只检查视频时间戳与首帧，速度较快，但不能证明后续帧均可解码。

- 检查反馈采样缺口、视频帧过旧、时间倒退/时钟不一致、交集为空、必需 topic/文件缺失、消息类型/维度、非有限数值、夹爪标定和单位、EEF/wrench 坐标系及四元数。
- 完整视频检查覆盖解码失败、帧与消息数量/顺序不一致、视频尺寸中途变化，以及不同录制的尺寸不一致。不推断未记录的真实曝光时间，也不能判定视觉内容或示范动作是否正确。
- 命令暂停记为 `command_pause_filled`（info），说明使用同帧 state 补齐；首尾静止裁剪记为 `stationary_edges_trimmed`（info）。这些不会将轨迹判为异常。
- `observation_gap` / `video_gap` 报告每个受影响信号的绝对纳秒起止时间、相对交集起点的秒数、间隔、阈值及受影响输出帧数。关节/夹爪反馈检查完整裁剪前交集；其他观测仅在影响保留帧时阻止转换，位于输出范围外的间隔记为 warning。
- 一条轨迹报错后继续检查其余轨迹；数值与视频独立检查，同次多路观测缺口逐项列出。结构性读取错误保留原始原因，无法读取的分支不声称完成后续检查。报告的 `completed_checks` 列出实际完成的检查项。

JSON 报告包含版本、配置、逐轨迹 `valid` / `issues`、视频解码帧数，以及通过轨迹的对齐审计和输出帧数。退出码：`0` 全部通过，`1` 存在异常轨迹，`2` 参数/路径或报告写入失败。检测本身不修复、移动或删除原始数据。
...
| `observation.action` | float32[16]，与 state 对应 | 有新鲜命令时使用记录的关节/夹爪目标；命令暂停时填同帧相应 state |
...
5. EEF 姿态使用最短路径 SLERP，并使输出四元数符号连续；不直接对四个分量线性插值或做普通低通。正常发布的关节指令和夹爪目标使用最近一次 `timestamp <= t` 的命令，不使用未来指令或对指令做低通。
6. 视频同样取 `timestamp <= t` 的最近一帧，四路采用完全相同的原始帧索引。30→50 FPS 会重复画面；不会声称获得了 50 Hz 相机采样，也不会生成运动插值画面。
7. 遥操作未移动某只手臂时，其命令可以停止发布。关节命令年龄超过 100 ms、夹爪目标年龄超过 150 ms 后，该侧 action 填为**同一输出时刻的对应 state**，即保持当前反馈位置；不是沿用旧目标，也不是填未来命令。四路命令独立判断，新命令出现即恢复使用。阈值分别由 `--command-max-age-ms`、`--gripper-max-gap-ms` 调整；正常采样间隔内继续零阶保持命令，不要求每个 50 Hz 网格点都有原始消息。
8. **从不拆 episode，也不删除中间网格点。** 命令停发不会触发失败。裁剪后的区间内，实际反馈采样间隔超过 `--max-gap-ms`、或视频所选帧年龄超过此值时，明确报错，不推测缺失反馈。保留 `--gap-policy error`；旧的 `split` 选项会被拒绝。整个必需 topic 缺失仍报错，此规则只补录制中间的命令暂停。
9. 默认 `--trim-stationary`：以关节和夹爪反馈判断首尾静止。首尾各 200 ms 的反馈中位数作为端点姿态，任一关节偏移超过 `--motion-joint-threshold-rad`（默认 0.002 rad，约 0.115°），或任一夹爪偏移超过 `--motion-gripper-threshold`（默认归一化行程 0.01），即视为运动。保留动作边界外一个采样点，只裁两端，所有字段和四路视频共享同一窗口。按位移而非逐帧速度判断，慢动作的累计位移也能触发；阈值以内的细微移动与噪声不能区分，可调低阈值。`--no-trim-stationary` 可关闭裁剪。整条均无可辨认运动时明确报错，不静默丢弃录制。

裁剪后的每条轨迹统一生成从 0 开始的 `timestamp=k/fps`，`frame_index` 连续，`index` 全局连续。中途暂停保留原有时长。MP4 帧数严格等于表格行数；标准 N 帧视频显示时长为 N/fps，而最后一帧呈现时间是 (N−1)/fps。

静止裁剪前会检查完整候选交集内的关节/夹爪反馈：缺少反馈的开头或结尾不能直接视为静止并删除。反馈不满足采样间隔限制时仍报错。修改观测间隔限制 `--max-gap-ms` 不会改变命令补齐阈值 `--command-max-age-ms`。

对齐报告保存每路起止时间、原始交集、首尾裁剪帧数/秒数/阈值，以及每路命令用 state 补齐的帧数和绝对时间范围。`grid_frames = kept_frames + trimmed_frames`，中间 `dropped_frames` 始终为 0。缓存每条轨迹的 `alignment.npz` 保存原始绝对时间、原始视频包索引、所选视频时间，可逐行追溯。报告随 `meta/info.json` 的 `tianji` 字段进入输出；完整逐行追溯数组保留在准备缓存中。
